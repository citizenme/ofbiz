...
<fo:block font-size="8pt">
    ...
    <fo:list-block provisional-distance-between-starts=".5in">
        ...
        <#if companyRegNo?exists>
        <fo:list-item>
            <fo:list-item-label>
             	<!-- Not working and life's too short... -->
                <!-- fo:block>${uiLabelMap.RemoteIntegrationCompanyRegNo}</fo:block -->
                <fo:block>Reg.No:</fo:block>
            </fo:list-item-label>
            <fo:list-item-body start-indent="body-start()">
                <fo:block>${companyRegNo}</fo:block>
            </fo:list-item-body>
        </fo:list-item>
        </#if>
        <#if phone?exists>
